...
static int ashmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	vm_flags &= ~calc_vm_may_flags(~asma->prot_mask);
...
/*
 * We hope to completely eliminate the use of ashmem_strinker in the future, 
 * as we have already migrated to the new API. However, 
 * since the migration of L886-L887 has not yet been completed, 
 * we will still keep it here.
*/
...
{
	int ret = -ENOMEM;
	struct shrinker * __maybe_unused ashmem_shrinker;
	...
	ashmem_shrinker = shrinker_alloc(0, "ashmem");
	if (!ashmem_shrinker) {
		pr_err("Failed to allocate ashmem shrinker!\n");
	} else {
