...
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 148)
	mm_segment_t old_fs;
#endif
	int ret = -1;
...
#else
	ret = inode_permission(&init_user_ns, d_inode(path.dentry), MAY_ACCESS);
#endif
	path_put(&path);
	return ret;
}
...
#else
		ret = vfs_mkdir(&init_user_ns, d_inode(path.dentry), dentry, BBOX_DIR_LIMIT);
#endif
		if (ret && ret != -EEXIST)
...
#else
		ret = vfs_unlink(&init_user_ns, d_inode(parent), dentry, NULL);
#endif
		dput(dentry);
	}
