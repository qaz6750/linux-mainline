...
static int goodix_ts_pinctrl_init(struct goodix_ts_core *core_data)
{
	...
	core_data->pinctrl = devm_pinctrl_get_select_default(core_data->ts_dev->dev);
if (IS_ERR(core_data->pinctrl)) {
   dev_err(core_data->ts_dev->dev, "Unable to get pinctrl\n");
...
static int goodix_bl_state_chg_callback(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (val != BACKLIGHT_PLATFORM)
		return NOTIFY_OK;
	if (data && core_data) {
		blank = *(int *)(data);
		ts_info("%s val:%lu, blank:%u\n", __func__, val, blank);
		if (blank == BACKLIGHT_RAW && (atomic_read(&core_data->suspend_stat) && core_data->fod_status)) {
...
#else
		//core_data->gtp_tp_class = class_create(THIS_MODULE->name, "touch");
#endif
